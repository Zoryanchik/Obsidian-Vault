![[Pasted image 20251103195503.png]]![[Pasted image 20251103195102.png]]![[Pasted image 20251103195113.png]]## 🧩 **Question 24**
...
- As packets move through the network:
    ...
    - When the packet **finally reaches the destination host** (but to a high-number UDP port that isn’t open), the destination sends back **ICMP Type 3 (Destination Unreachable)** with **Code 3 (Port Unreachable)**.
...
- TTL matching (option c) is **not reliable**, and matching in order (option d) isn’t always accurate in real networks.
...
- All traceroute probes come from the **same source IP** (the host running traceroute). ✅
    
- They are all destined to the **same target IP address**, but with **different TTL values**. ✅
    
- Each probe uses a **different destination port** (incremented each time) to uniquely identify the probe. ✅
    
- The **TTL value changes** with each request (starting from 1 and increasing), so option (c) is ❌.
    
- The **source and destination ports** are **not constant** (so option b is ❌).